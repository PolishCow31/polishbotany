...
- **Integration:** `.fox-img` → `image-rendering:pixelated`, height 52px; controller restored to walk; **fox PNG URLs now cache-busted by the manifest `?v=`** (`FOXV`) — the old 220x146 frames were stale-cached, so without this the new sprite wouldn't show. `git rm img/fox-source.png` (drawnlegs leftover, unused now).
- **Verified:** frames load fresh 38x27, pixelated, render crisp + charming on the hero, 0 console errors. **NOT motion-verified by me** (headless rAF pause) → awaiting Christian's eyes on the live walk before calling it done. A proper curled-sit pose is a possible follow-up (idle is standing for now).
- Supersedes the session-31 "FOX WALK SOLVED via IK legs" block below — that approach is dead. `docs/fox-walk-gait-spec.md` gait rules still hold (the pixel walk follows them).
...
- **Winner = "drawnlegs":** keep the gorgeous nano-banana fox BODY, erase the (frozen) legs along a tuned belly cutoff, and draw 4 legs **procedurally** — real 2-bone law-of-cosines **IK** (knees/elbows bend), feet plant + push BACKWARD through stance then lift in a sin-arc swing, correct lateral-sequence phasing (contacts 1/3/5/7, near-hind leads near-fore by 2, far = near +4, ≥2 feet always down), far legs behind body / near in front for depth.
- **Reproducible:** `scripts/build_fox_walk.py` reads pristine `img/fox-source.png` (a saved copy of the clean fox) → writes `img/fox-walk-1..8.png` in cycle order + `docs/fox-walk-frames.png` reference strip. Re-run anytime. All 8 frames verified DISTINCT (no paw-dance), load with 0 console errors, and read cleanly at the true 48px display size (checked).
- **Controller:** `WALK` array now plays the 8 frames in natural cycle order (was a scrambled order that hid the old frames' jitter). The existing distance-locked stepping / bottom-anchor / flip / sit logic is unchanged — it just got correct frames. Motion can't be seen in the headless preview (rAF pauses when hidden) but plays on a real device.
- **`docs/fox-walk-gait-spec.md` updated** from "parked, build when Fable 5 returns" → "SOLVED via procedural IK". Supersedes the [[reference_nanobanana_no_gait]] "wait for a stronger model" conclusion — we solved it with code, not a model.
